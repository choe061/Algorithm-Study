 ...
 * 내가 생각한 방법
 * 반드시 방문해야하는 지점(=K)들 중 가장 작은 고도를 가진 지점과 가장 높은 고도를 가진 지점을 구해 그 사이의 고도로만 이동한다 MAX - MIN = 답
 * 예외로 그 사이에 값이 없을 경우는 현재 지점과 8방향 탐색 지점 중 가장 차가 적은 곳으로 이동 -> 이동 후에는 MAX값과 MIN 값을 갱신한다
 *
 * 범위안에 있는 값이면 가중치가 0
